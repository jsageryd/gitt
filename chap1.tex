...
Tamagoyaki Inc.
create\footnote{creates} software for turning a standard PC into a media center.
...
\thoughtbreak
\footnote{I like these `thoughtbreaks', they look nice}
...
``This was almost a major embarrassment for Tamagoyaki Inc.\footnote{After all Inc.s, put an explicit space (backslash + space) to prevent the extra wide space. Also fix my accidental newlines after Inc.}\ We need to ensure this doesn't happen again. The demonstration at the trade show was close to a complete failure. Luckily someone had the good sense to bring a backup machine.''
He turned to Markus.
...
``Version 2.3 --\footnote{Use double dash in the source and it'll look better (and be proper), like this one which I corrected (cf. next one)} fixed bug a'' only means something to you if you know what bug a is and something like ``Version 2.3 - fixed bug a(2)'' is even worse.
...
\begin{trenches}
``So really it seems like the only real solution to this problem bar Klaus' suggestions of reducing the workforce to only one developer, thank you Klaus,''\footnote{lol} Klaus nodded in acknowledgement back to John, ``is to implement a version control system.''
...
Due to it's\footnote{I know I said I won't look at language, but} distributed nature, when pushing code to a more central location for others to pull from, developers can choose which branches they want to push, allowing code to be experimented with privately.

The implementation of branching in Git is fast.
Due to the fact that repositories are stored locally, the speed of creating a local branch is limited only by the speed of the disks on a local machine.\footnote{Sounds like branching is a heavy operation (that Git manages to handle); perhaps hint that branches are extremely lightweight}

\index{Staging}\subsection{Staging}

Git deals with commits\footnote{I guess everyone knows what a commit is? Perhaps ask around and see if it's obvious terminology or not} differently to most other version control systems by introducing the staging area.
...
The Integration Manager will then review the changes that the developers have made and will merge them into their\footnote{perhaps use `his', sentence is slightly ambiguous; a bit hard to follow, at least for me} own local repository.
...
This is all due to the fact that Git does 99\% of all of it's\footnote{ouch, my eye} operations locally.
...
One of the most common Shell Extensions for Git is the TortoiseGit\footnote{which doesn't seem to be very mature yet, perhaps be careful with recommending it at this point} interface which integrates itself into Windows Explorer, allowing a user to right click on an entity whilst inside a git working tree, and be presented with a context sensitive menu for VCS operations.

\index{user interface!command line}\subsubsection{Command Line Interface (CLI)}

The command line interface is favour\footnote{favoured?} by many developers as they can script with it and can see exactly what is going on, often in much more detail that with a GUI.
The CLI gives total control over the product\footnote{sounds like you are talking about hair wax when you refer to it as `the product'} and it is worth noting that almost all version control systems start life as command line driven interfaces.
...
The team nodded and everyone walked out of the board room\footnote{wow, no objections from anyone} except John.
...
The main thing to bear in mind is that Git is a Distributed Version Control system\footnote{capitalise `system' $\rightarrow$ `System' too perhaps (DVCS)}.
While this is so, it is equally important to remember that it can be used in the same workflow models as Centralised Version Control systems\footnote{hmm, or perhaps not capitalise the other words instead}.
...
Most of the time people will get more frustrated about not know\footnote{I'm trying hard not to but I keep spotting the typos} what they should or shouldn't be doing, rather than that they do or don't have access to certain things.
