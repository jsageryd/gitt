...
It is the first place\footnote{note that things defined in this config overrides any other config} git looks for upon invocation.
If this file is not present, Git will inspect \texttt{\textasciitilde\footnote{tilde character could look better}/.gitconfig}.
...
\index{HEAD}\textbf{HEAD} - This file is a\footnote{contains a} pointer to the parent\footnote{parent commit of my current branch\ldots, so e.g. \texttt{master\textasciicircum}? write that it points to the branch itself, or a commit object if HEAD is detached} commit of your current branch.
...
The remaining portion of the SHA-1 hash\footnote{sometimes you say SHA-1 value, sometimes SHA-1 hash; you should be consistent here} is used to name the file.\footnote{unless it's a pack\ldots}
...
We are going to look now at three\footnote{perhaps note that there are four types in total but that the tag object will be explained later} types of object that Git places in this directory.
...
This object stores\footnote{either list all of the things it stores, or say `among other things' or so; also, there are two dates so there is no `the date'} information about the committer, the date, a link to the previous commit object and most importantly a link to the tree object of the current commit.
...
\index{blob!internals}\item\textbf{blob} - A blob object actually contains the data\footnote{`contains the actual data'?} that resides inside the file.
There is one object generated per revision of a file.
However, if exactly the same data resides in two separate files, then there will only be one object created and that object will be referenced by two\footnote{or one\ldots} trees.
...
Below\footnote{below is Figure~1, but I understand} is a consolidated list of all the objects in the repository.
...
By George!\footnote{\ldots{}George?} Looking at the list above, we can see that there is one file which fits the bill.
...
to\footnote{capitalise} start with, let us run the Linux \texttt{file}\footnote{users of msysgit do not have the \texttt{file} command} command on it, to see what it makes of it.
...
Git stores its objects by Zlib compressing\footnote{`Zlib-compressing'? also check that Zlib is the proper capitalisation} them, which is why it is difficult for the \texttt{file} command to make any sense out of them.
...
Anyone who has spent any time with Git\footnote{do you mean *nix?} will know what the \texttt{cat} command does.
\texttt{cat} is used to display the contents of files.
In Git, this command is used to display the contents, type or size of a repository object, be it either of type commit, tree or blob\footnote{or tag}.
...
The commands that are readily available to the end user are called Porcelain\footnote{with a capital P?} commands because they have been refined and are simple to use, think Tap and Sink.
...
Why don't we run the command against that file\footnote{object} too?
...
Git will sometimes clean itself\footnote{\ldots{}well the repo at least} up and perform packing operations.
