...
\subsection{A Brief History Of Version Control}\footnote{I'm curious, does one normally capitalise even words like `of'?}
...
Rest assured, you are not the first person to do this, and in 1972, someone called Marc J.\footnote{sorry, line break inserted by mistake at these places, you should replace with an explicit space (backslash + space) after the period}
...
In 1982, Walter F.\footnote{same here, sigh}
Tichy released RCS, standing for Revision Control System.
It was intended to be a free and off more functionality than SCCS.\footnote{odd sentence}
...
However, RCS, like it's\footnote{ouch, my other eye} predecessor SCCS, has no way of dealing with groups of files.
...
CVS\footnote{what does `CVS' stand for?} was created in 1986, and began life as a set of shell scripts to operate on multiple files, using RCS to perform the actual repository management.
...
Three prominent systems that were released within a short time of each other and\footnote{and?} ClearCase, VSS and Perforce.
...
In Subversion, people often fine\footnote{how much?} the implementation of tags -\footnote{double-dash!} names that point to specific points in the history of a repository, an issue.
In SVN, a tag is actually a branch.
What makes this different to other systems such as Git and its predecessor CVS, which literally point to a specific commit in the tag, SVN actually creates a snapshot of the file system, albeit using cheap branching.\footnote{odd sentence}
Whilst this is lightweight on the repository, it is incredibly heavyweight on the client.\footnote{I don't quite get what this actually means in practice}
...
Mercurial or Hg as it is often known, began development on April 19th of the same year and was started by Matt Mackall with largely the same goals as Git.\footnote{You might want to very carefully check this information, I have been told by some that the development of Git actually started after that of Mercurial, and that it even `stole' some of its concepts. It may be difficult to find the real truth.}
...
Some that have been mentioned by people over the years are its particular steep learning curve for basic understanding.\footnote{This was true in the early days of Git; today I would argue that it is not much different in terms of difficulty compared to other popular systems; though I may be biased}
